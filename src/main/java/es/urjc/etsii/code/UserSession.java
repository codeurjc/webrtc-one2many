...
public class UserSession {
	...
	private One2ManyHandler handler;
	...
	public UserSession(WebSocketSession wsSession, String sessionNumber,
			One2ManyHandler handler, int bandwidth) {
	...
	public void initViewer(UserSession presenterSession, JsonObject jsonMessage)
		...
		log.info("[Session number {} - WS session {}] Init viewer(s)",
				sessionNumber, wsSession.getId());
		...
		WebRtcEndpoint inputWebRtcEndpoint = presenterSession
				.getWebRtcEndpoint();
		connectMediaElements(inputWebRtcEndpoint, webRtcEndpoint);
	...
	private void connectMediaElements(MediaElement input, MediaElement output) {
			...
			public void run() {
				...
				for (int i = 0; i < fakeClients; i++) {
					...
					executor.execute(new Runnable() {
						...
						public void run() {
							try {
								addFakeClient(j, inputWebRtcEndpoint,
				...
				if (removeFakeClients) {
					...
					for (List<MediaElement> list : mediaElementsInFakeMediaPipelineMap
						...
						for (int i = 0; i < list.size() / 2; i++) {
							...
							for (int j = 0; j < 2; j++) {
								MediaElement mediaElement = list.get(2 * i + j);
	...
	private void addFakeClient(int count, WebRtcEndpoint inputWebRtc,
			int fakeClientsPerInstance) {
		log.info("[Session number {} - WS session {}] Adding fake client #{}",
				sessionNumber, wsSession.getId(), count);
		...
		if (mediaElementsInFakeMediaPipelineMap
			...
			fakeClientNumber = currentMediaElementList.size() / 2;
			log.debug(
		...
		connectMediaElements(inputWebRtc, fakeOutputWebRtc);
